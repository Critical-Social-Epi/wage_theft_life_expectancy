...
point_ests <- function(pctiled1, pctiled2, rooted, fractioned){
  binded %>% 
    ...
    ungroup() %>%
    group_by(gnd) %>%
    mutate(meaned=meaned, #mean LE in each scenario; pooled SE is pctile SE divided by sqrt of sample size (100 rows per gender in full sample; 10 rows per gender within deciles)
           ...
           diff_lower=diff-(1.96 * (sdd/sqrt(rooted))),
           diff_upper=diff+(1.96 * (sdd/sqrt(rooted))),
