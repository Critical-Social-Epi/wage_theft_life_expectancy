...
#merge in ACS data on population counts in 2014; don't need to adjust for inflation since only need to exclude those with $0 incomes
fread(here("ACS_IPUMS.csv")) %>%
  filter(FTOTINC>0 & AGE==40) %>% #family income yields similar estimates to household income, but I assume "family income" is a bit closer to Chetty's definition since joint tax filers are likely to be related
  mutate(gnd=ifelse(SEX==1, "Men", "Women")) %>%
  group_by(gnd) %>%
  summarise(ACS_pop=sum(PERWT)) %>%
  right_join(chetty, by="gnd") -> chetty #~2.05-2.1 million of each gender
```
...
```{r warning=FALSE, message=FALSE}
kable(chetty[1:10,c(1,3:11)], digits=2) %>% 
...
#print
kable(theft, col.names=c("Family income", "Percent wage theft", "Annual underpayment", "Gender"), digits=2) %>% 
  kable_styling("striped") %>%
...
binded %>%
  ...
  left_join(binded, 'type') -> binded

#plot
ggplot(subset(binded, pctile<=50), aes(x=hh_inc/1000, color=type, group=type)) +
  geom_density() +
...
binded %>% 
  ...
  ungroup() %>%
  group_by(gnd) %>%
  mutate(meaned=meaned, #mean LE in each scenario; pooled SE is pctile SE divided by sqrt of sample size (100 rows per gender)
         meaned_lower=meaned-(1.96 * (sdd/sqrt(100))),
         meaned_upper=meaned+(1.96 * (sdd/sqrt(100))),
         diff=meaned-first(meaned), #diff in mean LE across scenarios; first is observed LE (alphabetical order); pooled SE for diff is same as above
         diff_lower=diff-(1.96 * (sdd/sqrt(100))),
         diff_upper=diff+(1.96 * (sdd/sqrt(100))),
         diff_pop=diff*ACS_pop,
         diff_lower_pop=diff_lower*ACS_pop,
         diff_upper_pop=diff_upper*ACS_pop) %>% 
  dplyr::select('gnd', 'type', 'meaned', 'meaned_lower', 'meaned_upper', 'diff', 'diff_lower', "diff_upper", 'diff_pop', 'diff_lower_pop', "diff_upper_pop") %>%
  arrange(desc(gnd), type) -> overall
...
binded %>% 
  ...
  ungroup() %>%
  group_by(gnd) %>%
  mutate(meaned=meaned, #mean LE in each scenario; pooled SE is pctile SE divided by sqrt of sample size (10 rows per gender)
         meaned_lower=meaned-(1.96 * (sdd/sqrt(10))),
         meaned_upper=meaned+(1.96 * (sdd/sqrt(10))),
         diff=meaned-first(meaned), #diff in mean LE across scenarios; first is observed LE (alphabetical order); pooled SE for diff is same as above
         diff_lower=diff-(1.96 * (sdd/sqrt(10))),
         diff_upper=diff+(1.96 * (sdd/sqrt(10))),
         diff_pop=diff*ACS_pop*0.1,
         diff_lower_pop=diff_lower*ACS_pop*0.1,
         diff_upper_pop=diff_upper*ACS_pop*0.1) %>% 
  dplyr::select('gnd', 'type', 'meaned', 'meaned_lower', 'meaned_upper', 'diff', 'diff_lower', "diff_upper", 'diff_pop', 'diff_lower_pop', "diff_upper_pop") %>%
  arrange(desc(gnd), type) -> affected
...
binded %>% 
  ...
  mutate(meaned=meaned, #mean LE in each scenario; pooled SE is pctile SE divided by sqrt of sample size (10 rows per gender)
         ...
         diff_upper_pop=diff_upper*ACS_pop*0.1) %>% 
  dplyr::select('gnd', 'type', 'meaned', 'meaned_lower', 'meaned_upper', 'diff', 'diff_lower', "diff_upper", 'diff_pop', 'diff_lower_pop', "diff_upper_pop") %>%
  arrange(desc(gnd), type) -> unaffected

#print
kable(rbind(overall, affected, unaffected), digits=c(2,2,2,2,2,2,2,2,0,0,0), 
...
#difference in mean life expectancy across scenarios at each decile
binded %>% 
  ...
  ungroup() %>%
  group_by(gnd, decile) %>%
  mutate(diff=meaned-first(meaned), #diff in mean LE across scenarios; first is observed LE (alphabetical order); pooled SE for diff is same as above
         diff_lower=diff-(1.96 * (sdd/sqrt(10))),
         diff_upper=diff+(1.96 * (sdd/sqrt(10))),
         diff_pop=diff*ACS_pop*0.1,
         diff_lower_pop=diff_lower*ACS_pop*0.1,
  ...
  dplyr::select('gnd', 'type', 'decile', 'diff', 'diff_lower', "diff_upper", 'diff_pop', 'diff_lower_pop', "diff_upper_pop", "ACS_pop") -> deciles

#plot diffs
p1 <- ggplot(deciles, aes(x=decile, y=diff, group=type, color=type, fill=type)) + 
  facet_wrap(~gnd) +
  geom_hline(yintercept=0, lty=2, color="darkgrey") +
  geom_pointrange(aes(y=diff, ymin=diff_lower, ymax=diff_upper), size=0.5, fatten=3, position=position_dodge(width=0.8)) + 
...
#plot years extended
p2 <- ggplot(deciles, aes(x=decile, y=diff_pop, group=type, color=type, fill=type)) + 
  facet_wrap(~gnd) +
  geom_hline(yintercept=0, lty=2, color="darkgrey") +
  geom_pointrange(aes(y=diff_pop, ymin=diff_lower_pop, ymax=diff_upper_pop), size=0.5, fatten=3, position=position_dodge(width=0.8)) + 
  scale_color_manual(values=c("#6A3D9A", "#33A02C", "#FF7F00"), labels=c('Scenario 1', 'Scenario 2', 'Scenario 3')) +
  scale_fill_manual(values=c("#6A3D9A", "#33A02C", "#FF7F00"), labels=c('Scenario 1', 'Scenario 2', 'Scenario 3')) +
...
ggsave('appendix_1a.png', height=4, width=4.25, dpi=600)
```
...
binded %>% 
  ...
  mutate(diff=meaned-first(meaned), #diff in mean LE across scenarios; first is observed LE (alphabetical order); pooled SE for diff is same as above
         ...
         diff_upper_pop=diff_upper*ACS_pop*0.1) %>% 
  filter(type!="Observed") %>%
  dplyr::select('gnd', 'type', 'decile', 'diff', 'diff_lower', "diff_upper", 'diff_pop', 'diff_lower_pop', "diff_upper_pop") -> deciles
...
ggsave('appendix_1b.png', height=6, width=8.5, dpi=600)
```
...
p1 <- ggplot(deciles, aes(x=decile, y=diff, group=type, color=type, fill=type)) + 
  facet_wrap(~gnd) +
  geom_hline(yintercept=0, lty=2, color="darkgrey") +
  geom_pointrange(aes(y=diff, ymin=diff_lower, ymax=diff_upper), size=0.5, fatten=2, position=position_dodge(width=1.0)) + 
  scale_color_manual(values=c("#CAB2D6", "#6A3D9A", "#B2DF8A", "#33A02C", "#FDBF6F", "#FF7F00")) +
...
p2 <- ggplot(deciles, aes(x=decile, y=diff_pop, group=type, color=type, fill=type)) + 
  facet_wrap(~gnd) + 
  geom_hline(yintercept=0, lty=2, color="darkgrey") +
  geom_pointrange(aes(y=diff_pop, ymin=diff_lower_pop, ymax=diff_upper_pop), size=0.5, fatten=2, position=position_dodge(width=1.0)) + 
  scale_color_manual(values=c("#CAB2D6", "#6A3D9A", "#B2DF8A", "#33A02C", "#FDBF6F", "#FF7F00")) +
  scale_fill_manual(values=c("#CAB2D6", "#6A3D9A", "#B2DF8A", "#33A02C", "#FDBF6F", "#FF7F00")) +
  ylab("Years of life expectancy extended") +
  xlab("Household income decile") +
...
ggsave('appendix_3.png', height=6.25, width=12, dpi=600)
```

## City-specific estimates

Analyzed Detroit and NYC as examples of disparate effects of scenarios geographically.
...
#merge in ACS data on population counts; don't need to adjust for inflation since only need to exclude those with $0 incomes
fread(here("ACS_IPUMS.csv")) %>%
  filter(FTOTINC>0 & AGE==40 & (MET2013==19820 | MET2013==35620)) %>% 
  mutate(gnd=ifelse(SEX==1, "Men", "Women"),
...
```{r warning=FALSE, message=FALSE}
kable(chetty_msa_long, digits=2) %>% 
  kable_styling('striped')  %>%
  scroll_box(width = "100%", height = "500px") 
```
...
#print
kable(theft %>% arrange(state, desc(gnd), hh_inc_cat), digits=2) %>% 
  kable_styling("striped") %>%
  scroll_box(width = "100%", height = "500px") 
...
binded %>% 
  ...
  ungroup() %>%
  group_by(gnd, czname) %>%
  mutate(meaned=meaned, #mean LE in each scenario; pooled SE is pctile SE divided by sqrt of sample size (20 rows per gender)
         meaned_lower=meaned-(1.96 * (sdd/sqrt(20))),
         meaned_upper=meaned+(1.96 * (sdd/sqrt(20))),
         diff=meaned-first(meaned), #diff in mean LE across scenarios; first is observed LE (alphabetical order); pooled SE for diff is same as above
         diff_lower=diff-(1.96 * (sdd/sqrt(20))),
         diff_upper=diff+(1.96 * (sdd/sqrt(20))),
         diff_pop=diff*ACS_pop,
         diff_lower_pop=diff_lower*ACS_pop,
         diff_upper_pop=diff_upper*ACS_pop) -> overall

#point estimates for difference in mean life expectancy within affected group
binded %>% 
  filter(pctile<=4) %>%
  group_by(type, gnd, czname) %>%
  summarise(meaned=mean(sim_1),
  ...
  ungroup() %>%
  group_by(gnd, czname) %>%
  mutate(meaned=meaned, #mean LE in each scenario; pooled SE is pctile SE divided by sqrt of sample size (2 rows per gender)
         meaned_lower=meaned-(1.96 * (sdd/sqrt(4))),
         meaned_upper=meaned+(1.96 * (sdd/sqrt(4))),
         diff=meaned-first(meaned), #diff in mean LE across scenarios; first is observed LE (alphabetical order); pooled SE for diff is same as above
         diff_lower=diff-(1.96 * (sdd/sqrt(4))),
         diff_upper=diff+(1.96 * (sdd/sqrt(4))),
         diff_pop=diff*ACS_pop*0.2,
         diff_lower_pop=diff_lower*ACS_pop*0.2,
         diff_upper_pop=diff_upper*ACS_pop*0.2) -> affected
...
binded %>% 
  ...
  mutate(meaned=meaned, #mean LE in each scenario; pooled SE is pctile SE divided by sqrt of sample size (2 rows per gender)
         ...
         diff_pop=diff*ACS_pop*0.2,
         diff_lower_pop=diff_lower*ACS_pop*0.2,
         diff_upper_pop=diff_upper*ACS_pop*0.2) -> unaffected
...
#print
kable(binded_ests, digits=c(2,2,2,2,2,2,2,2,2,0,0,0),
...
ggsave('appendix_4b.png', height=6, width=9, dpi=600)
```

## Yearly life expectancy inequity in first versus tenth income deciles
